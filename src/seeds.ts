...
function generateUniqueUsername() {
  // Generate a base username using faker
  const baseUsername = faker.internet.userName();

  // Generate a random string or number to append for uniqueness
  const randomString = Math.random().toString(36).substring(2, 8); // Generates a 6-character random string

  // Combine the base username and random string to create a unique username
  const uniqueUsername = `${baseUsername}_${randomString}`;
  const username = uniqueUsername.substring(0, 8);
  return username;
}
